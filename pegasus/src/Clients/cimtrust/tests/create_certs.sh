...
# certs that are not specifically created as part of the Makefile.  This
# includes:
...
openssl x509 -req -set_serial 0 -days 3650 -in testinter.csr -CA testchca.cert -CAkey testchca.key -CAcreateserial -out testinter.cert
# rm testinter.csr

# create the leaf key, csr and signed cert
openssl genrsa -out testleaf.key 2048
openssl req -config ${OPENSSL_CNF} -new -key testinter.key -out testleaf.csr < testleaf.txt
